...
cmake ${BUILD_PATH}/.. -DCMAKE_INSTALL_PREFIX=${INSTALL_PATH} -DCMAKE_C_FLAGS=-fPIC -DCMAKE_CXX_FLAGS=-fPIC -DTBB_LOCATION=${REZ_TBB_ROOT} -DPTEX_LOCATION=${REZ_PTEX_ROOT} -DGLEW_LOCATION=${REZ_GLEW_ROOT} -DGLFW_LOCATION=${REZ_GLFW_ROOT} -DNO_OPENGL=1 -DNO_CUDA=1 -DNO_OPENCL=1
